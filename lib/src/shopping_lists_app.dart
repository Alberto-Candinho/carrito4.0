...
class _ShoppingListsAppState extends State<ShoppingListsApp> {
  ...
  void initState() {
    super.initState();
    initMQTT();
    _shoppingListsBloc = new ShoppingListsBloc(
  ...
  void dispose() {
    for (ShoppingList list in _shoppingLists.props)
      _manager.unsubscribe(list.listId);
    _manager.disconnect();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(providers: [
      ...
      BlocProvider<TrolleyBloc>(create: (_) => _trolleyBloc)
  ...
  void _onReceivedMqttMessage(String messageTopic, String messagePayload) {
    for (ShoppingList list in _shoppingLists.props) {
      if (list.listId == messageTopic)
        _processListMessage(list, messagePayload);
      else if (list.listId + "/trolley" == messageTopic)
        _processTrolleyMessage(list, messagePayload);
    }
  }
  ...
  void _processListMessage(
      ShoppingList affectedList, String messagePayload) async {
    ...
    print(
        "[${affectedList.listName}] Mensaxe mqtt recibido. Nome de lista recibido: $receivedListName. Productos recibidos: ${receivedProductsIds.toString()}");
    ...
    if (receivedListName != affectedList.listName)
      _shoppingListBloc
          .add(RenameList(list: affectedList, newName: receivedListName));
  }

  void initMQTT() async {
    _manager = MQTTManager("carrito4.duckdns.org", "test1", _currentAppState,
        _onReceivedMqttMessage);
