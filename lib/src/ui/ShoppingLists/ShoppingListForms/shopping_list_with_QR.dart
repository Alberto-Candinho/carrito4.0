  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Column(children: [
          Row(
            children: [
              ...
              Expanded(
                ...
                child: SingleChildScrollView(
                    ...
                    child: BlocBuilder<ShoppingListBloc, ShoppingListState>(
                      ...
                      return Text(
                          (state is ShoppingListAvailable)
                              ? list.listName
                              : "Loading lists info",
              ...
              IconButton(
                ...
                onPressed: () {
                  if (list.listId != null)
                    Share.share("shoppinglists.com/list/" + list.listId);
                  else
                    showDialog(
                        context: context,
                        builder: (_) => new AlertDialog(
                              title: new Text("Non se pode compartir a lista"),
                              content: new Text("A lista: " +
                                  list.listName +
                                  " non se pode compartir porque non se pudo xerar un identificador para a mesma"),
                            ));
                },
              )
            ],
          ...
          BlocBuilder<TrolleyBloc, TrolleyState>(builder: (context, state) {
            print("AQUI => " + state.toString());
            if (state is Unconnected)
              return QrImage(data: list.listId);
            else {
              if (list.isInTrolley()) {
                return ShoppingListInTrolley(
                    list: list, trolley: state.props[0]);
              } else {
                return Text("Xa estas comprando outra lista\n");
              }
            }
          })
        ]));
